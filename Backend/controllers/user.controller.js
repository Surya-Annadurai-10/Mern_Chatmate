...
export const SendFriendRequestController = async (req , res) =>{
 try {
  ...
  if(alreadyFriends) res.status(401).json({
    success : false,
  ...
  try {
    ...
    if(!friendRequest){
     ...
     if(friendRequest.recipient != req.user.id){
      ...
      const updatedFriendListSender = await UserModel.findByIdAndUpdate(friendRequest.sender,{
        $addToSet : {friends : friendRequest.recipient}
      })


       const updatedFriendListRecipient = await UserModel.findByIdAndUpdate(friendRequest.recipient,{
...
export const OngoingGetFriendRequestController = async (req , res) =>{
  try {
    ...
    res.status(200).json({
     ...
     console.log("Error while listing the friend Requests status" , error.message);
    res.status(400).json({
      success : false,
      message : error.message
    })
